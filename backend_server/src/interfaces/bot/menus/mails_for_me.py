...
router = Router(name=callback_data.mails_for_me.name)



@router.message(Command(callback_data.mails_for_me.name))
...
@router.callback_query(
    callback_data.MenuItem.filter(F.name == callback_data.mails_for_me.name)
)
...
async def get_text(msg: types.Message):
    mails = await MailsService.get_written_for_me(user_id=msg.from_user.id)
    logging.warn(mails)
    if mails == []:
