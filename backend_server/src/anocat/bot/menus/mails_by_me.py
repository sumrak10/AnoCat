...
router = Router(name=callback_data.mails_by_me.name)



@router.message(Command(callback_data.mails_by_me.name))
...
@router.callback_query(
    callback_data.MenuItem.filter(F.name == callback_data.mails_by_me.name)
)
...
async def get_text(msg: types.Message):
    ...
    if mails == []:
        return "Писем пока что нет! Переходите по топикам друзей и оставляйте \
            анонимные или публичные письма ;)"
    text = "Письма:\n"
    for mail in mails:
